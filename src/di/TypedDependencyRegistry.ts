...
export class TypedDependencyRegistry {
    ...
    public static get(klass: Klass) {
        ...
        if (component.params && component.params.length > 0) {
            params = component.params.map((param, index) => {
                ...
                return TypedDependencyRegistry.get(param);
        ...
        TypedDependencyRegistry.set(klass, instance);
    ...
    public static registerHandler(target: any, key: string, index?: number) {
        ...
        if (typeof index !== 'undefined') {
            ...
            component.paramHandlers.set(index, () => TypedDependencyRegistry.get(paramKlass));

        } else {

            const klass = Reflection.getType(target, key);
            component.propertyHandlers.set(key, () => TypedDependencyRegistry.get(klass));
        }
    }
