...
describe("[Integration] Param", () => {
  ...
  @RestController('/ParamIntegration')
  class UserController {
   ...
   const options = {
    ...
    it('should get HeaderParam in the action', async () => {
        const response = await HttpHelper.request("get", `http://localhost:${nodeServer.address().port}/ParamIntegration/users`, options);
        expect(response.statusCode).to.be.equal(200);
        expect(response.body).to.be.equal(123);
    });

    it("should get PathParam in the action", async () => {
        const response = await HttpHelper.request("get", `http://localhost:${nodeServer.address().port}/ParamIntegration/users/1`, options);
        expect(response.statusCode).to.be.equal(200);
        expect(response.body).to.be.equal(1);
    });

    it('should get BodyParam in the action', async () => {
        const response = await HttpHelper.request("post", `http://localhost:${nodeServer.address().port}/ParamIntegration/users`, options);
        expect(response.statusCode).to.be.equal(201);
        expect(response.body).to.be.equal(1);
    });

    it('should get QueryParam in the action', async () => {
        const response = await HttpHelper.request("get", `http://localhost:${nodeServer.address().port}/ParamIntegration/users/active`, options);
        expect(response.statusCode).to.be.equal(200);
    ...
    it('should get complex object in the action', async () => {
        ...
        const response = await HttpHelper.request("patch", `http://localhost:${nodeServer.address().port}/ParamIntegration/users/1`, options);
        expect(response.statusCode).to.be.equal(200);
