...
import { Controller, Get, Res, Req, Middleware, IMiddleware, Next } from '../src';
...
test('global middleware', async t => {
    const response1 = await helper.getAxios().get('/global');
    t.is(response1.status, 200);
    t.is(response1.headers['global'], 'true');

    const response2 = await helper.getAxios().get('/partial');
