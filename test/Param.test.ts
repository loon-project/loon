...
bootstrapServer(({getAxios}) => {
    ...
    test('should get header param', async t => {
        const response = await getAxios().get('/header', {
            headers: {
                'authorization': '123'
            }
        })
    ...
    test('should get path param', async t => {
        const response = await getAxios().get('/path/1')
        t.is(response.status, 200)
        t.is(response.data, true)
    })

    test('should get query param', async t => {
        const response = await getAxios().get('/query?id=1')
        t.is(response.status, 200)
        t.is(response.data, true)
    })

    test('should get body param', async t => {
        const response = await getAxios().post('/body?id=1', "1", {
            headers: {
